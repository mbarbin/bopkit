...
let find_files_in_cwd_by_extensions ~extensions =
  ...
  List.filter ls_dir ~f:(fun file ->
    ...
    && List.exists extensions ~f:(fun extension ->
      Filename_base.check_suffix file extension))
;;
    ...
    let rec find_fix_point ~num_steps ~filename =
      ...
      let result =
        ...
        | Ok true ->
          ...
          else find_fix_point ~num_steps:(Int.succ num_steps) ~filename:temp_file
      in
      ...
      (let open Command.Let_syntax in
       ...
       fun () ->
         ...
         List.iter files ~f:(fun filename ->
           print_endline (Printf.sprintf "================================: %s" filename);
