  ...
  Command.basic
    ...
    (let open Command.Let_syntax in
     ...
     Error_log.report_and_exit ~config (fun error_log ->
       ...
       let p = Parsing_utils.parse_file_exn (module Visa_syntax) ~filename ~error_log in
       print_s [%sexp (p : Visa.Program.t)];
       return ()))
;;

let fmt_cmd =
  Fmt_command.fmt_cmd (module Visa.Program) (module Visa_syntax) (module Visa_pp.Program)
;;
...
let disassemble_cmd =
  Command.basic
    ...
    (let open Command.Let_syntax in
     ...
     Error_log.report_and_exit ~config (fun error_log ->
       let open Or_error.Let_syntax in
       let machine_code = Visa.Machine_code.of_text_file_exn ~filename ~error_log in
       let program =
         Visa.Executable.Machine_code.disassemble machine_code ~filename ~error_log
       in
       Format.printf "%a%!" Pp.to_fmt (Visa_pp.Program.pp program);
       return ()))
;;
