...
let fragments_of_cds ~(cds : Bopkit_circuit.Cds.t) =
  let fresh_ram =
    let index = ref (-1) in
    fun () ->
      Int.incr index;
  ...
  Array.iteri cds ~f:(fun gate_index gate ->
    ...
    | External { loc; _ } ->
      Err.raise ~loc [ Pp.text "External blocks are not supported by bop2c." ]
    | Regt -> ()
...
let emit_c_code ~(circuit : Bopkit_circuit.Circuit.t) ~to_:oc =
