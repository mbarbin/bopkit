...
let fragments_of_cds ~(cds : Bopkit_circuit.Cds.t) ~error_log =
  ...
  Array.iteri cds ~f:(fun gate_index gate ->
    ...
    | Clock -> emit (Pp.verbatim (sprintf "%s = 1;" outputs.(0)))
    | Gnd -> emit (Pp.verbatim (sprintf "%s = 0;" outputs.(0)))
    | Vdd -> emit (Pp.verbatim (sprintf "%s = 1;" outputs.(0)))
    | External { loc; _ } ->
      Error_log.raise
        error_log
        ~loc
      ...
      Queue.enqueue
        ...
        (Pp.verbatim
           (if String.equal inputs.(1) "1"
            then sprintf "%s = %s;" s_index inputs.(0)
            else sprintf "if (%s) %s = %s;" inputs.(1) s_index inputs.(0)))
    | _ -> ());
  Fragments.T
...
let emit_c_code ~(circuit : Bopkit_circuit.Circuit.t) ~error_log ~to_:oc =
  let filename = circuit.filename in
  ...
  in
  let pp =
    [ Pp.verbatim (sprintf "/* Program generated by bop2c */") ++ Pp.newline
    ; Pp.verbatim (sprintf "/* Original circuit : %s , Main = \"%s\" */" filename main)
      ++ Pp.newline
    ; Pp.newline
    ...
    |> Pp.concat
  in
  Out_channel.output_string oc (pp |> Fmt_command.pp_to_string);
